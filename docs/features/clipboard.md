...
- **Clipboard writes stamp a private `internalType` marker** so the poller skips UltraCMD's own writes.
...
- **A `.file` entry references the file where it lies and never copies it.** Its absolute path is
  ...
  UltraCMD did not write. `kind` is a plain `TEXT` column, so the case cost no migration; an older
...
- **No recognition ever runs in the app process.** `ClipboardTextWorker` spawns one bundled
  ...
  allocations do not accumulate in UltraCMD. The helper is handed a path and answers with text.
...
tree: a harness that touched `NSPasteboard.general` would land in the reader's own running UltraCMD
...
re-capturing UltraCMD's own writes, every write stamps a private `internalType` marker on the
...
`isCapturing` flag that `prepareForUltraCMDPasteboardMutation` reads — so a paste UltraCMD performs
...
A file copied in Finder is recorded as a reference, never as a copy: UltraCMD writes nothing to
...
copy-only for the reverse reason: the path is the user's own file, and UltraCMD must not move it.
