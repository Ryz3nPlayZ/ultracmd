...
UltraCMD has one app-wide provider layer for features that need text generation. AI Chat chooses its
...
- **Every request carries UltraCMD's own preamble, and the user's text goes after it.**
  ...
  UltraCMD over anything else. It is not shown in the pane, and `AIPreamble.swift` holds the only
...
- **Installed commands reuse their own login.** UltraCMD launches the user's `codex`, `claude` or
  `opencode` executable without asking for or storing another key. Codex inherits the user's normal
  home and credential-store setting; Claude and OpenCode inherit their normal configuration. UltraCMD
...
- **Claude and OpenCode are text transports, not agents.** Claude runs one turn with no tools, MCP
  ...
  deny-all permissions, disabled sharing and a private working directory; UltraCMD deletes the session
...
completion path. Gemini requests identify UltraCMD through `x-goog-api-client`; OpenRouter requests
carry the app title.

**The Ollama preset is the loopback case made one click.** "Add Ollama (Local)" in Settings opens
the connection editor pre-filled with `OpenAI Compatible` and `http://localhost:11434/v1` — plain
HTTP is permitted there because the endpoint policy allows it only for loopback hosts, no key is
needed, and model discovery reads the daemon's OpenAI-shaped `/v1/models`, so installed models
fill the picker the same way a hosted catalog does. Nothing else about Ollama is special-cased:
it is an OpenAI-compatible connection that happens to live on this Mac.

Each connection has an ordered, deduplicated list of exact model identifiers. While its editor is open,
UltraCMD asks the configured provider for the models available to the entered key and uses the result
...
manually. UltraCMD does not ship or guess an API catalog that can become stale. Codex gets its models
...
the only destination UltraCMD can offer the switch to honestly. Picking `None` sends
...
It uses the system SQLite already linked by UltraCMD, stores no provider credentials, and repairs a
...
- Install and sign in to each supported command outside UltraCMD, choose one of its discovered models,
...
commands are never installed by UltraCMD; Settings links to their own install docs and offers a sign-in
...
server uses the same login and credential store as the user's normal Codex command. UltraCMD supplies
...
bubble's status line. System messages become developer instructions alongside UltraCMD's fixed
...
UltraCMD's instructions and bounded conversation history as stdin, consumes newline-delimited JSON,
...
| OpenRouter | `plugins: [{id: "web"}]` — OpenRouter's own layer, any model | `image_url` part, only for models whose catalog lists the `image` modality | never yet — its catalog publishes a `file` modality UltraCMD does not read | `tools` + `role: "tool"` turns |
...
`AIModelCapabilities.documents` is true only for the two HTTP shapes whose bodies UltraCMD writes;
...
tools, run commands or touch files. That is a sandbox boundary on a local CLI, not UltraCMD
