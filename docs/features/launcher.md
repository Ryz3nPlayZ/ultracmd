...
Stats, UltraCMD itself, Mission Control, Siri, Time Machine, Screenshot, System Information, Font
...
| --- | --- | --- |
| `.userAlias` | the alias the user typed in UltraCMD, for any entry kind | literal |
...
and its `url` carries the destination instead of the catalog's `ultracmd://` placeholder. Nothing
...
`Fallback` (`Launcher/Model/`) is the whole vocabulary — `.builtin(Builtin)` for the shipped
...
| Quick Add Event | the default calendar, natural language parsed (`QuickCaptureCoordinator`) | `calendarEnabled` |
| New Reminder | Reminders, date and title split from the line | always |
| Start Timer | a countdown notification, duration read from the line | always |
| Search Maps | `maps://` — a search, or a route when the line says "directions to" | always |
| Search the Web | the chosen engine's search URL (`WebSearchEngine`) | always |
| a quicklink | its first `{argument}` | `quicklinksEnabled`, and the link has a placeholder |

The five capture fallbacks share `Features/QuickCapture/`: the parser is pure
(`quick-capture-test`), EventKit and notification consent is requested by the capture itself, and
the Quick Add Event row rides the Calendar feature's existing consent — `calendarEnabled` gates it,
so it never holds a permission of its own. Search the Web's engine is Settings ▸ Fallbacks ▸ Web
Search (`webSearchEngine`, part of the settings backup like every preference).
...
`SystemActionCatalog` is a Foundation-only inventory of the macOS actions UltraCMD exposes. Its
...
opened it. Volume and mute actions also show UltraCMD's transient volume HUD, since macOS only draws
...
UltraCMD. See [quick-actions.md](quick-actions.md).
...
> **Invariant:** `Tests/fuzz-test.swift` compiles the real `UltraCMD/Features/Launcher/Model/SearchRelevance.swift`, so
...
- **Quit All Applications** a system action. `AppLauncher.quitAllTargets()` is the
  policy (every `.regular` app except Finder — `terminate()` only relaunches it — and UltraCMD,
