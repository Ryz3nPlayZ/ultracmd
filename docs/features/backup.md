...
Export and import of UltraCMD's own data as a single `.ultracmd` file, plus the entry point for
...
- **No absolute path may enter a `.ultracmd`.** A clip's `imagePath` names a file on the Mac that wrote
...
| `Model/BackupArchive.swift` | Directory ⇄ `.ultracmd`; the only file importing `AppleArchive` |
...
| `Service/RaycastImportReader.swift` | Raycast → UltraCMD field mapping |
...
The old flat `UltraCMD-Settings-*.json` export is gone rather than deprecated, and nothing reads it.
