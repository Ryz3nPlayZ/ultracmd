...
Debug (`com.ultracmd.app.dev`), beta, and stable therefore never share snippet files. The storage
...
frontmatter keeps the same identity. Renaming a file outside UltraCMD appears as deletion of the old
...
an independently selectable category. UltraCMD reads `title`, `text` and the optional `keyword` from
...
suffixes as snippets created in UltraCMD, and duplicate keywords are preserved.
...
there are no aliases, so a key UltraCMD does not know names itself in the error.
...
A token UltraCMD cannot parse — an unknown name, an unknown modifier, a duplicated or unsupported
...
duplicates resolve by file identity. UltraCMD-tagged synthetic events are ignored.
...
After either launcher or keyword delivery is confirmed, UltraCMD may show a brief non-activating,
...
for permissions or activates UltraCMD. Failed, cancelled, rejected, or prompt-cancelled expansions do
...
Longer or multiline fallback text uses a temporary paste. UltraCMD snapshots every item, type and data
payload, then **lends a board holding nothing but the expansion** — one item carrying the plain text
and UltraCMD's own marker type — and restores by rewriting the snapshot whole.
...
previous copy instead. A representation UltraCMD cannot rewrite to mean the expansion is one it must
...
synchronizes to UltraCMD's ownership changes so temporary or restored text is not added as new history.
...
any pasteboard restoration still owned by UltraCMD.
...
non-ignored input to `cancelAutomaticExpansion`, and UltraCMD's own tagged synthetic events classify
...
between revalidation and mutation, so UltraCMD does not claim that impossible guarantee.
