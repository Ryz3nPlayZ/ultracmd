...
Quicklinks reuse UltraCMD's one template engine — the same
...
UltraCMD's own dialog and leaves no partial state.
...
independently selectable category. UltraCMD reads `name`, `link`, `createdAt` and the optional
