...
Create the `UltraCMD Self-Signed` code-signing identity once — builds sign with it, which is what keeps
...
```sh
open UltraCMD.xcodeproj    # then ⌘R
```

Or from the command line:

```sh
xcodebuild -project UltraCMD.xcodeproj -scheme UltraCMD -configuration Debug build
```
...
`UltraCMD.xcodeproj` is committed and generated from `project.yml` via XcodeGen — after changing
...
Debug builds are a separate channel: **`UltraCMD Dev.app`**, bundle id `com.ultracmd.app.dev`. Every
...
- The dev build asks for Accessibility on its own the first time, and starts with **no** hotkeys bound
  ...
  and the `UltraCMD Self-Signed` identity keep the TCC grant alive.
...
xcodebuild -project UltraCMD.xcodeproj -scheme UltraCMD -configuration Debug \
    -derivedDataPath build/DerivedData build 2>&1 | tee /tmp/ultracmd-build.log
./Scripts/sync-lsp.sh /tmp/ultracmd-build.log
```
...
**Build UltraCMD.app (debug)** task (⌘⇧B) and **F5** re-run the script on every build, so new and
...
```sh
./Scripts/format.sh            # format UltraCMD/ and Tests/ in place
...
```sh
node Scripts/gen-emoji.js            # -> UltraCMD/Features/Emoji/Model/EmojiData.generated.swift
node Scripts/gen-currencies.js       # -> UltraCMD/Features/Calculator/Model/CurrencyData.generated.swift
node Scripts/gen-countries.js        # -> UltraCMD/Features/Calculator/Model/CountryZoneData.generated.swift
```
