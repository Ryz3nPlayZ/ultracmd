...
How UltraCMD is wired together. Per-feature internals live in [features/](README.md#features);
...
`UltraCMDApp` (`@main`) declares only two `MenuBarExtra` scenes — UltraCMD's own item and the
...
- **The main menu** — shaped by `UltraCMDApp`'s `.commands`, which rebinds ⌘Q to Close Settings. It is
