...
```sh
./Scripts/build-dmg.sh            # -> build/UltraCMD-<version>.dmg (version from project.yml)
./Scripts/build-dmg.sh 0.5.7      # -> build/UltraCMD-0.5.7.dmg
```
...
Every release publishes two assets from one build: `UltraCMD-<version>.dmg`, which people download by
hand and which the cask installs, and `UltraCMD-<version>.zip`, which the in-app updater installs. The
...
A stable release publishes two more from the `universal` job, `UltraCMD-Universal-<version>.dmg` and
...
brew never reports UltraCMD outdated, never re-downloads it, and never rolls a self-updated copy back.
...
- **channel** — `beta` or `stable`. Each builds a distinct app (`UltraCMD Beta.app` / `UltraCMD.app`)
  with its own bundle id, alongside the local `UltraCMD Dev.app`. Beta gets an auto-incrementing
...
`ARCHS="arm64 x86_64"` and attaches `UltraCMD-Universal-<version>.dmg` / `.zip` to the release the
first job created, then bumps `ultracmd-universal`. macOS 26 is the last release that boots on Intel,
...
- **The body is split by `<!-- ultracmd:install -->`.** Everything above it is the changelog;
...
the docs section) and deploys it to Cloudflare at `https://ultracmd.dev/` on every push to `main`
...
makes GitHub 301 every `https://abue-ammar.github.io/ultracmd/<path>` to `https://ultracmd.dev/<path>`
at its own edge. Enable it once via **Settings → Pages → Source = GitHub Actions** and
**Custom domain = ultracmd.dev**. See [website/README.md](../website/README.md).
...
bucket behind `cdn.ultracmd.dev`, and is mirrored by its own
