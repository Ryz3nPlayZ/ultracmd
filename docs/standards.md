...
How code in UltraCMD is written. This is **guidance** — it describes what the codebase already looks
...
one-line call into a layer nobody dares delete. UltraCMD has no external API, no plugin surface and one
...
- **One folder per feature** under `UltraCMD/Features/<Name>/`, holding everything that feature owns —
