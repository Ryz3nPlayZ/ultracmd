...
The design system for UltraCMD's UI, written so an agent restyling or extending it stays consistent
with what's already there. This documents **UltraCMD as built** — every rule here maps to code in
`UltraCMD/`. `DesignSystem/Theme.swift` is the single design-token source.
...
UltraCMD is a **command palette**: a borderless floating panel whose surface is just the
...
- **No `NSAlert` or system popovers.** Every confirmation, failure report, value prompt and transient readout is UltraCMD's own SwiftUI surface (see "Dialogs & HUD"). An Aqua alert on an alpha-over-vibrancy app reads as a different product, and its `runModal` run loop keeps Carbon hotkeys firing underneath.
...
Source: `UltraCMD/DesignSystem/Theme.swift`.
...
UltraCMD owns its dialogs; `NSAlert` is never used. `DialogController` is owned by `AppCore` (the
...
writes `AppleShowScrollBars = WhenScrolling` into UltraCMD's own defaults domain, which outranks the
...
countdown and buttons around it are UltraCMD's own. Its buttons are a **deliberate copy** of
...
Its placeholder is drawn by UltraCMD, not by the field's `prompt` — an `NSTextField` renders a prompt
...
[features/palette.md](features/palette.md#the-placeholder-is-ultracmds-not-the-fields).
