...
How to check that a change holds up. UltraCMD has no XCTest target and no UI tests: the automated half
...
| Pure-layer purity | `grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' UltraCMD/Features/*/Model/` |
...
seconds to about 15. `ULTRACMD_TEST_JOBS=1` forces it back to one at a time. Each result is numbered
...
than `ULTRACMD_TEST_TIMEOUT` seconds (default 300) is killed and reported as timed out. Parallelism is safe
...
is the trap: a running UltraCMD records every write to it as a genuine copy, so a fixture left there
...
| `quick-capture-test` | `QuickCapture/Model/QuickCaptureParser.swift` and the web-search engines' URL building |
...
```sh
grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' UltraCMD/Features/*/Model/
```
...
xcodebuild build -project UltraCMD.xcodeproj -scheme UltraCMD -configuration Debug \
  CODE_SIGNING_ALLOWED=NO
xcodebuild build -project UltraCMD.xcodeproj -scheme UltraCMD -configuration Release \
  CODE_SIGNING_ALLOWED=NO
find ~/Library/Developer/Xcode/DerivedData -name "UltraCMD*.app" -maxdepth 6 -print -quit
```
...
`Platform/Signposts.swift` emits eight intervals on the `com.ultracmd.perf` subsystem: `AppCore.start`,
...
```sh
swiftc -O -swift-version 6 UltraCMD/Platform/Signposts.swift \
...
```sh
swiftc -O -swift-version 6 UltraCMD/Features/Calculator/Model/*.swift \
...
```sh
swiftc -O -swift-version 6 UltraCMD/Platform/PasteboardFiles.swift \
    UltraCMD/Features/Clipboard/Model/{ClipboardStore,ClipboardFilter,ColorValue,ColorFormat,ColorSpaces}.swift \
...
```sh
swiftc -O -swift-version 6 UltraCMD/Features/Emoji/Model/{EmojiCatalog,EmojiData.generated}.swift \
    UltraCMD/Features/Emoji/Service/{EmojiIndex,FrequentEmojiStore}.swift \
    UltraCMD/Features/Launcher/Model/SearchRelevance.swift UltraCMD/Platform/{AppPaths,Memo}.swift \
...
Run against the **Debug channel** (`UltraCMD Dev.app`, `com.ultracmd.app.dev`). It has its own prefs,
...
- Paste from clipboard history lands in that app, not in UltraCMD
...
- ⌃⇧X asks first, through UltraCMD's own dialog; Cancel and Esc both leave every entry in place
...
- A shortcut that fails shows UltraCMD's dialog with the tool's error
...
- Delete confirms through UltraCMD, moves the file to Trash, and selecting another note never loses an
  unsaved edit
...
- Hiding restores the previous external app or UltraCMD window
...
- Calendar in Menu Bar on Disabled: the calendar item is gone and UltraCMD's own item is unaffected;
...
- Export produces a `.ultracmd`; import applies it and reports a per-category summary
...
- Cancelling the save panel leaves nothing in `~/Library/Caches/com.ultracmd.app.dev/backup-staging/`
...
```sh
rm -rf ~/Library/Caches/com.ultracmd.app.dev
rm -rf "$HOME/Library/Application Support/com.ultracmd.app.dev"
defaults delete com.ultracmd.app.dev 2>/dev/null || true
tccutil reset Accessibility com.ultracmd.app.dev 2>/dev/null || true
```
...
- Nothing was written outside `com.ultracmd.app.dev/`. Channel isolation is not negotiable — a Dev build
