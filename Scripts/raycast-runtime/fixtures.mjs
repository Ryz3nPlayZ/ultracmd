...
export default function Command() {
  ...
  const parts = [
    ...
    fileURLToPath(pathToFileURL("/tmp/a?b.png")),
    fileURLToPath(pathToFileURL("/Applications/UltraCMD Beta.app")),
...
export default async function Command() {
  const target = join(tmpdir(), "ultracmd-fixture-index.json");
...
export async function runFixtures() {
  ...
  await run("Form fields and submit", formSource, "view", async (harness) => {
    ...
    check("field has a change handler", !!field.props.onUltraCMDChange?.$fn);

    harness.dispatch("s1", field.props.onUltraCMDChange.$fn, ["Grace"]);
    await wait();
  ...
  await run("Navigation push and pop", navigationSource, "view", async (harness) => {
    ...
    harness.call('__ultracmd.popNavigation("s1")');
    await wait();
