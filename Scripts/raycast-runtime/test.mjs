...
const runtimePath = [
  resolve("UltraCMD/Resources/RaycastRuntime.generated.js"),
  resolve("../../UltraCMD/Resources/RaycastRuntime.generated.js"),
  fileURLToPath(new URL("../../UltraCMD/Resources/RaycastRuntime.generated.js", import.meta.url)),
].find(existsSync);
...
export function createHarness({ onRender, onFail, verbose = false, stubs = {} } = {}) {
  ...
  const host = {
    ...
    startTimer(id, ms, repeats) {
      const fire = () => runInContext(`__ultracmd.fireTimer(${JSON.stringify(id)})`, context);
  ...
  function settle(callId, ok, value) {
    runInContext(
      `__ultracmd.settle(${JSON.stringify(String(callId))}, ${ok}, ${JSON.stringify(value === undefined ? "" : JSON.stringify(value))})`,
      context,
    );
  }

  context.__ultracmdHost = host;
  // Mirrors what Swift installs: compile the extension's CJS body in global scope.
  context.__ultracmdCompile = (code, filename) =>
    runInContext(
  ...
  return {
    ...
    boot(config) {
      return runInContext(`__ultracmd.boot(${JSON.stringify(JSON.stringify(config))})`, context);
    },
    start(sessionId, code, filename, dirname, mode, ctx) {
      return runInContext(
        `__ultracmd.start(${JSON.stringify(sessionId)}, ${JSON.stringify(code)}, ${JSON.stringify(filename)}, ${JSON.stringify(dirname)}, ${JSON.stringify(mode)}, ${JSON.stringify(JSON.stringify(ctx))})`,
        context,
      );
    },
    dispatch(sessionId, handlerId, args = []) {
      return runInContext(
        `__ultracmd.dispatch(${JSON.stringify(sessionId)}, ${JSON.stringify(handlerId)}, ${JSON.stringify(JSON.stringify(args))})`,
        context,
      );
    },
    stop(sessionId) {
      runInContext(`__ultracmd.stop(${JSON.stringify(sessionId)})`, context);
