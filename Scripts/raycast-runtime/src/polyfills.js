...
class UltraCMDHeaders {
  constructor(init) {
    this._map = new Map();
    if (init instanceof UltraCMDHeaders) {
...
class UltraCMDBlob {
  ...
  slice(start = 0, end = this.size, contentType = "") {
    ...
    return new UltraCMDBlob([this._bytes.subarray(Math.min(from, to), to)], { type: contentType });
  }
}

function blobPartToBytes(part) {
  if (part instanceof UltraCMDBlob) return part._bytes;
...
class UltraCMDFile extends UltraCMDBlob {
...
class UltraCMDFormData {
  constructor() {
    this._entries = [];
    // Header and body must carry the same boundary, so it lives on the instance, not the encoder.
    this._boundary = `----UltraCMDFormBoundary${Math.random().toString(36).slice(2, 18)}`;
  }
...
function formDataValue(value, filename) {
  if (!(value instanceof UltraCMDBlob)) return String(value);
  if (value instanceof UltraCMDFile && filename === undefined) return value;
  return new UltraCMDFile([value], filename ?? "blob", { type: value.type });
}
...
function formDataToBytes(form) {
  ...
  for (const [name, value] of form._entries) {
    const disposition =
      value instanceof UltraCMDBlob
        ? `; name="${escapeFormName(name)}"; filename="${escapeFormName(value.name)}"`
        : `; name="${escapeFormName(name)}"`;
    const type = value instanceof UltraCMDBlob ? `Content-Type: ${value.type || "application/octet-stream"}\r\n` : "";
    chunks.push(utf8Encode(`--${form._boundary}\r\nContent-Disposition: form-data${disposition}\r\n${type}\r\n`));
    chunks.push(value instanceof UltraCMDBlob ? value._bytes : utf8Encode(value));
...
if (!g.Blob) g.Blob = UltraCMDBlob;
if (!g.File) g.File = UltraCMDFile;
if (!g.FormData) g.FormData = UltraCMDFormData;

class UltraCMDResponse {
  ...
  constructor(body = null, init = {}, url = "") {
    ...
    this.headers = new UltraCMDHeaders(init.headers);
    this.url = url;
  ...
  clone() {
    ...
    return new UltraCMDResponse(this._bytes ?? this._stream, { status, statusText, headers }, this.url);
  }
  async arrayBuffer() {
    this.bodyUsed = true;
  ...
  async blob() {
    return new UltraCMDBlob([await this.bytes()], { type: this.headers.get("content-type") ?? "" });
  }
}

class UltraCMDRequest {
  constructor(input, init = {}) {
    if (input instanceof UltraCMDRequest) {
      this.url = input.url;
      this.method = init.method || input.method;
      this.headers = new UltraCMDHeaders(init.headers || input.headers);
...
async function ultracmdFetch(input, init = {}) {
  const request = input instanceof UltraCMDRequest ? input : new UltraCMDRequest(input, init);
...
class UltraCMDDOMException extends Error {
...
if (!g.DOMException) g.DOMException = UltraCMDDOMException;
...
function bodyToBytes(body) {
  ...
  if (body instanceof UltraCMDBlob) return body._bytes;
  if (body instanceof UltraCMDFormData) return formDataToBytes(body);
...
function bodyContentType(body) {
  ...
  if (body instanceof UltraCMDFormData) return `multipart/form-data; boundary=${body._boundary}`;
  if (body instanceof UltraCMDBlob) return body.type || null;
  return null;
}
...
if (!g.fetch) {
  g.fetch = ultracmdFetch;
  g.Headers = UltraCMDHeaders;
  g.Response = UltraCMDResponse;
  g.Request = UltraCMDRequest;
}
