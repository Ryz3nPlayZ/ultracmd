// Entry point. Installs the polyfills and the module registry, then exposes `__ultracmd` — the only
...
defineModule("react-dom", {
  render: () => {
    throw new Error("react-dom is not available — UltraCMD renders extensions natively.");
  },
...
const hostCalls = {
  render: (sessionId, json) => globalThis.__ultracmdHost.render(sessionId, json),
  failed: (sessionId, message) => globalThis.__ultracmdHost.failed(sessionId, message),
  navigationDepthChanged: (sessionId, depth) =>
    globalThis.__ultracmdHost.navigationDepthChanged(sessionId, String(depth)),
  finished: (sessionId) => globalThis.__ultracmdHost.finished(sessionId),
};
...
setFieldCommandHandler((command, fieldId) => {
  globalThis.__ultracmdHost.fieldCommand(String(command), String(fieldId ?? ""));
});

globalThis.__ultracmd = {
