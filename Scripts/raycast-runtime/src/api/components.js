  ...
  return function Field(props) {
    ...
    return h(
      type,
      {
        ...rest,
        value,
        onUltraCMDChange: (next) => {
          const decoded = type === "Form.DatePicker" ? decodeDate(next) : next;
          setValue(decoded);
          props.onChange?.(decoded);
        },
        onUltraCMDBlur: props.onBlur ? () => props.onBlur({ target: { value } }) : undefined,
        onUltraCMDFocus: props.onFocus ? () => props.onFocus({ target: { value } }) : undefined,
      },
      props.children,
    );
  };
...
Action.PickDate = function ActionPickDate(props) {
  return h(Action, {
    ...
    onUltraCMDChange: (value) => props.onChange?.(decodeDate(value)),
  });
};
