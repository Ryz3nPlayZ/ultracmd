...
export function requireModule(name) {
  ...
  throw new Error(
    `Cannot find module '${key}'. UltraCMD provides React, @raycast/api and a subset of Node builtins — see docs/extensions.md.`,
  );
}
...
export function evaluateCommonJS(code, filename, dirname) {
  ...
  const factory = globalThis.__ultracmdCompile(code, filename);
