// The single seam between the JS runtime and Swift. Swift installs `__ultracmdHost` on the global
// before evaluating the bundle; everything else in here goes through these helpers.

const raw = globalThis.__ultracmdHost;
if (!raw) throw new Error("__ultracmdHost missing — the runtime was evaluated outside UltraCMD.");
...
/// `__ultracmd.settle`, so the JS thread never blocks waiting on the main actor.
