...
final class AppCore {
    ...
    let snippetListener = SnippetKeywordListener(
        syntheticEventTag: Paster.ultracmdEventTag)
    ...
    @ObservationIgnored private(set) lazy var calendarCoordinator = CalendarCoordinator(
        ...
        paletteCoordinator: paletteCoordinator, core: self)
    @ObservationIgnored private(set) lazy var quickCaptureCoordinator = QuickCaptureCoordinator(
    ...
    func start() {
        Signposts.interval("AppCore.start") {
            ...
            dictation.onUnavailable = { [weak self] message in
                self?.showMessage(message, tone: .danger)
            }
            applyAppearance()
            ...
            updateChecker.start()
            // No support nag in this fork: the window stays reachable from its command alone.
            supportReminders.onDue = nil
