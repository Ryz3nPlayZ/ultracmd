...
struct RootPaletteView: View {
    ...
    private var appMenuContent: PopoverMenuContent {
        PopoverMenuContent(items: [
            PopoverMenuItem(title: "About UltraCMD", systemImage: "info.circle") {
                core.settingsCoordinator.showAbout()
            },
            PopoverMenuItem(title: "Support UltraCMD", systemImage: "heart") {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 0) {
            ...
            if !isCollapsed, vm.mode == .launcher || vm.mode == .ai {
                headerGutter(width: metrics.spacing.md)
                DictationButton(
                    controller: core.dictation, action: core.dictation.toggle)
            }
        ...
        // A hidden palette has no field to type into, so the microphone stops with it.
        .onChange(of: vm.isVisible) { _, visible in
            if !visible { core.dictation.stop() }
        }
    }
