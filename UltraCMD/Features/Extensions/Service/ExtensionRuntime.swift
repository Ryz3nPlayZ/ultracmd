...
final class ExtensionRuntime: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.ultracmd.extensions.js", qos: .userInitiated)
    ...
    private func bootOnQueue(config: ExtensionBootConfig) throws {
        ...
        _ = context.objectForKeyedSubscript("__ultracmd")?
    ...
    func start(
        ...
        await onQueue { context in
            let compiled = context.objectForKeyedSubscript("__ultracmd")
    ...
    func popNavigation(session: String) async -> Bool {
        await withCheckedContinuation { continuation in
            queue.async {
                ...
                let result = context.objectForKeyedSubscript("__ultracmd")?
    ...
    /// Installs `__ultracmdHost` (the JS→Swift seam) and `__ultracmdCompile`.
    private func installHost(in context: JSContext) {
        ...
        context.setObject(host, forKeyedSubscript: "__ultracmdHost" as NSString)
        ...
        context.setObject(compile, forKeyedSubscript: "__ultracmdCompile" as NSString)
    }
    ...
    private func startTimer(id: String, milliseconds: Double, repeats: Bool) {
        ...
        timer.setEventHandler { [weak self] in
            ...
            _ = self.context?.objectForKeyedSubscript("__ultracmd")?
