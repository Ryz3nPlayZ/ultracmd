...
struct FallbacksSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("The engine the Search the Web fallback hands the query to.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var engineBinding: Binding<WebSearchEngine> {
        Binding(
            get: { core.settings.webSearchEngine },
            set: { core.settings.webSearchEngine = $0 })
    }
