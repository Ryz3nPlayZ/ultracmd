...
final class LauncherCoordinator {
    ...
    func runCommand(_ id: CommandID) {
        ...
        case .quickAddEvent, .newReminder, .startTimer, .searchMaps, .searchWeb:
            break  // Query-driven: each runs where the typed text is, never through this funnel.
        case .newEmail:
            dismissPalette()
            core.quickCaptureCoordinator.composeMail()
        case .newMessage:
            dismissPalette()
            core.quickCaptureCoordinator.composeMessage()
        case .newFaceTime:
            dismissPalette()
            core.quickCaptureCoordinator.callFaceTime()
        case .showNotes:
            dismissPalette()
