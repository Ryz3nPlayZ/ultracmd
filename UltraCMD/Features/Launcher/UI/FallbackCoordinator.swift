...
final class FallbackCoordinator {
    ...
    func run(_ fallback: Fallback, query: String) {
        ...
        case .builtin(.quickAddEvent): core.quickCaptureCoordinator.quickAddEvent(text: query)
        case .builtin(.newReminder): core.quickCaptureCoordinator.createReminder(text: query)
        case .builtin(.startTimer): core.quickCaptureCoordinator.startTimer(text: query)
        case .builtin(.searchMaps): core.quickCaptureCoordinator.searchMaps(query: query)
        case .builtin(.searchWeb): runWebSearch(query)
    ...
    private func runWebSearch(_ query: String) {
        guard let url = settings.webSearchEngine.url(for: query) else { return }
    ...
    private func isAvailable(_ builtin: Fallback.Builtin) -> Bool {
        ...
        // The event lands on the calendar the Calendar feature already holds consent for.
        case .quickAddEvent: return settings.calendarEnabled
        case .newReminder, .startTimer, .searchMaps, .searchWeb: return true
        }
    }
}
