...
extension AppEntry {
    ...
    init(_ layout: WindowLayout) {
        self.init(
            ...
            url: URL(string: "ultracmd://window-layout/" + layout.id.uuidString)!,
    ...
    init(_ size: CustomWindowSize) {
        self.init(
            ...
            url: URL(string: "ultracmd://window-size/" + size.id.uuidString)!,
    ...
    init(_ action: CustomQuickAction) {
        self.init(
            ...
            url: URL(string: "ultracmd://quick-action/" + action.id.uuidString)!,
    ...
    init(_ quicklink: Quicklink) {
        self.init(
            ...
            url: URL(string: "ultracmd://quicklink/" + quicklink.id.uuidString)!,
...
final class AppIndex {
    ...
    private static let systemActionEntries: [AppEntry] = SystemActionCatalog.all
        .map { command in
            AppEntry(
                ...
                url: URL(string: "ultracmd://system-action/" + command.id.rawValue)!,
    ...
    private static let allWindowCommandEntries: [AppEntry] = WindowCommandCatalog.all
        .map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "ultracmd://window-command/" + command.id.rawValue)!,
                bundleID: nil, kind: .windowCommand)
        }
        .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
    ...
    func setCustomCommands(_ commands: [CustomCommand]) {
        let entries = commands.filter(\.isEnabled).map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "ultracmd://custom-command/" + command.id.uuidString)!,
