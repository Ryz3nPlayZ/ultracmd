...
final class AppSettings {
    ...
    /// Which engine the "Search the Web" fallback hands the query to.
    var webSearchEngine: WebSearchEngine {
        didSet { defaults.set(webSearchEngine.rawValue, forKey: Key.webSearchEngine.rawValue) }
    }

    /// Follow macOS, or pin UltraCMD to one appearance. Applied by `AppCore.applyAppearance()`.
    ...
    /// Off by default: connecting a server is consent to run code UltraCMD did not write.
    var mcpEnabled: Bool {
    ...
    /// Off out of the box: on means UltraCMD may read a selection anywhere and type over it.
    ...
    /// For a toolchain UltraCMD doesn't know — mise or Nix shims are the common case.
