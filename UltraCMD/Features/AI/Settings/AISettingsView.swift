...
struct AISettingsView: View {
        ...
        Section {
            ...
            AIModelSelectionRows(
                ...
                modelLabel: {
                    ...
                    Text("Used by UltraCMD features unless they ask you to choose another model.")
                },
                effortLabel: {
    ...
    private var defaultModelFooter: String {
        ...
        return settings.defaultModel == nil
            ...
            : "UltraCMD contacts only the selected provider when an AI feature runs."
    }
    ...
    private var systemPromptSection: some View {
        ...
        return Section {
            Toggle(isOn: $settings.systemPromptEnabled) {
                ...
                Text("Off sends nothing ahead of your message, not even what UltraCMD says about itself.")
            }
            ...
            Text(
                "Your text is sent ahead of every message in every chat, after what UltraCMD "
    ...
    private var installedAISection: some View {
        Section {
            ...
            Text(
                "UltraCMD uses the Codex, Claude and OpenCode commands already installed and signed "
                    + "in on this Mac. UltraCMD never stores or asks for their API keys."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
        }
        ...
        if settings.enabledInstalledProviders.contains(kind) {
            ...
            case .notInstalled:
                LabeledContent {
                    ...
                    Text("UltraCMD could not find the \(kind.command) command.")
                }
            case .failed(let message):
                LabeledContent {
                    providerActions {
        ...
        Section {
            ...
            Button {
                editor = AIConnectionEditorTarget(
                    connection: AIConnection(
                        name: "Ollama", provider: .openAICompatible,
                        baseURL: "http://localhost:11434/v1"),
                    hasStoredKey: false, isNew: true)
            } label: {
                Label {
                    SettingsRowTitle(.aiAPIConnections, "Add Ollama (Local)")
                } icon: {
            ...
            Text(
                "OpenAI, Claude, Gemini and OpenRouter are presets. Custom OpenAI-compatible "
                    + "endpoints are supported too — the Ollama preset reads a local daemon at "
                    + "localhost:11434, no key needed. API keys stay in your login Keychain."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
        }
