...
final class CalendarStore {
    ...
    /// UltraCMD's own consent dialog has already been accepted by the time this runs.
    ...
    func createEvent(_ draft: EventDraft, now: Date) -> Bool {
        createEvent(
            title: draft.trimmedTitle, start: draft.start(from: now),
            duration: TimeInterval(draft.durationMinutes * 60))
    }

    /// The natural-language path: a parsed start date, which `EventDraft`'s offsets cannot carry.
    @discardableResult
    func createEvent(title: String, start: Date, duration: TimeInterval) -> Bool {
        let store = eventStore ?? EKEventStore()
        eventStore = store
