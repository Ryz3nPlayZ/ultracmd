...
final class TextInjector {
    ...
    /// A hotkey's target comes from `frontmostApplication`, which can be UltraCMD itself.
    ...
    func copySelection(
        ...
        clipboardManager.prepareForUltraCMDPasteboardMutation()
    ...
    private func restore(_ snapshot: PasteboardSnapshot, to pasteboard: any PasteboardAccess) {
        ...
        clipboardManager.synchronizeAfterUltraCMDPasteboardMutation(
    ...
    private func finish(_ lease: TemporaryPasteboardLease) {
        ...
        case .restored(let changeCount):
            ...
            clipboardManager.synchronizeAfterUltraCMDPasteboardMutation(changeCount: changeCount)
        case .superseded:
            break
        case .failed:
