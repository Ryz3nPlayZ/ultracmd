...
private enum CapsLockRemap {
    ...
    private static let queue = DispatchQueue(label: "com.ultracmd.capslock-remap", qos: .utility)
    ...
    private static func apply(_ mapping: String) {
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                NSLog("UltraCMD: hidutil remap exited %d", process.terminationStatus)
            }
        } catch {
            NSLog("UltraCMD: hidutil caps lock remap failed: %@", error.localizedDescription)
        }
    }
}
