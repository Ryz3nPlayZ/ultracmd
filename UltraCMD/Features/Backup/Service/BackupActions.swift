...
extension UTType {
    ...
    static let ultracmdBackup = UTType(exportedAs: "com.ultracmd.backup")
}
...
enum BackupActions {
    ...
    // MARK: - UltraCMD native (own file panels; dialogs come from `AppCore`)
    ...
    static func chooseBackupFile() -> URL? { chooseFile(ofType: .ultracmdBackup) }
    ...
    {
        guard let destination = chooseSaveLocation(named: "UltraCMD", type: .ultracmdBackup) else {
    ...
    private static let restartAfterImportText = "Quit and reopen UltraCMD to finish."
