...
enum ShellCommandRunner {
    ...
    private static let queue = DispatchQueue(
        label: "com.ultracmd.shell-command", qos: .userInitiated, attributes: .concurrent)
    ...
    nonisolated private static func execute(
        ...
        // Lets a shell config skip slow sections when UltraCMD is the caller.
        process.environment = ProcessInfo.processInfo.environment.merging(["ULTRACMD": "1"]) { _, new in
            new
        }
    ...
    nonisolated private static func shellArguments(
        ...
        [loadingShellEnvironment ? "-ilc" : "-lc", command, "ultracmd"] + arguments
    }
    ...
    private final class StreamCapture: @unchecked Sendable {
        ...
        static func make() -> StreamCapture? {
            let template = FileManager.default.temporaryDirectory
                .appendingPathComponent("ultracmd-command-stream.XXXXXX").path
