...
**UltraCMD targets one macOS — the current stable release — and nothing else.** macOS 26+, the Xcode 26
...
| Folder | Holds |
| --- | --- |
| `UltraCMD/App/` | `@main`, `AppDelegate`, `AppCore` — the composition root |
| `UltraCMD/DesignSystem/` | shared visual primitives; `Theme.swift` is the only design-token source |
| `UltraCMD/Platform/` | system shims: `Permissions`, `AppPaths`, `Signposts`, `NotificationToken`, … |
| `UltraCMD/Palette/` | the palette shell: panel, window controller, `RootPaletteView`, `PaletteScreen` |
...
- **UltraCMD presents its own dialogs — never `NSAlert` or a system popover.** A question
...
- **Debug builds are their own channel** — `UltraCMD Dev.app` / `com.ultracmd.app.dev` — so a local run
...
- **XcodeGen owns the project.** `UltraCMD.xcodeproj` is committed but generated from `project.yml`;
...
- `grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' UltraCMD/Features/*/Model/` returns nothing.
