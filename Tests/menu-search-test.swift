...
struct MenuSearchTests {
    ...
    static func targetClassify() {
        ...
        expect(
            classify("UltraCMD", isSelf: true, hasMenuBar: false) == .selfTarget,
            "self wins over the menu-bar check, since UltraCMD itself runs accessory")
        expect(
        ...
        expect(
            classify("UltraCMD", isSelf: true, isExcluded: true) == .selfTarget,
            "self still wins: UltraCMD has no menu to exclude in the first place")
    }
