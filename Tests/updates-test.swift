...
struct UpdatesTests {
    ...
    static func derivesChannels() {
        let stable = ReleaseChannel(bundleID: "com.ultracmd.app")
        let beta = ReleaseChannel(bundleID: "com.ultracmd.app.beta")
        let dev = ReleaseChannel(bundleID: "com.ultracmd.app.dev")
        ...
        expect(
            !(stable.updatesItself || beta.updatesItself || dev.updatesItself),
            "no channel updates itself: this fork publishes no release feed")
    ...
    static func entry(
        tag: String, prerelease: Bool, draft: Bool = false, assets: [String] = ["UltraCMD-x.zip"],
        body: String = "Notes."
    ) -> String {
        let list = assets.map {
            """
    ...
    static func rejectsUnusableFeeds() {
        ...
        expect(
            ReleaseFeed.newest(
                from: feed(entry(tag: "v0.3.0", prerelease: false, assets: ["UltraCMD-x.dmg"])),
    ...
    static func picksTheZipThisMacCanRun() {
        let both = feed(
            entry(
                ...
                assets: ["UltraCMD-0.3.0.zip", "UltraCMD-Universal-0.3.0.zip"]))
        expect(
        ...
        let thinOnly = feed(entry(tag: "v0.3.0", prerelease: false, assets: ["UltraCMD-0.3.0.zip"]))
        expect(
        ...
        let universalOnly = feed(
            entry(tag: "v0.3.0", prerelease: false, assets: ["UltraCMD-Universal-0.3.0.zip"]))
        expect(
