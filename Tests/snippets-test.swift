...
struct SnippetsTests {
    ...
    private static func testRaycastImport() {
        ...
        check(
            "Raycast import uses safe UltraCMD defaults",
    ...
    private static func testRepositoryStorage() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "ultracmd-snippets-tests-\(UUID().uuidString)",
            isDirectory: true)
        ...
        let stable = SnippetRepository(
            bundleIdentifier: "com.ultracmd.app",
            applicationSupportRoot: channelRoot)
        let beta = SnippetRepository(
            bundleIdentifier: "com.ultracmd.app.beta",
            applicationSupportRoot: channelRoot)
        let dev = SnippetRepository(
            bundleIdentifier: "com.ultracmd.app.dev",
            applicationSupportRoot: channelRoot)
    ...
    private static func testRepositoryConcurrency() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "ultracmd-snippets-concurrency-\(UUID().uuidString)",
            isDirectory: true)
        try fm.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: root) }
        ...
        boundaryEdit.text = "UltraCMD edit"
        do {
    ...
    private static func testCopySelectionFallback() async {
        ...
        let backing = NSPasteboard(name: .init("ultracmd-copy-tests-\(UUID().uuidString)"))
    ...
    private static func testDeliveryQueueAndPasteboard() async throws {
        ...
        let backingPasteboard = NSPasteboard(
            name: .init("ultracmd-snippets-tests-\(UUID().uuidString)"))
        ...
        check(
            "pasteboard restoration leaves no UltraCMD marker on the restored clipboard",
    ...
    private static func testStoreWatcher() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "ultracmd-snippets-watcher-\(UUID().uuidString)",
            isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let repository = SnippetRepository(
    ...
    private static func testDynamicPlaceholders() {
        ...
        // Raycast's snippet spelling resolves like UltraCMD's.
    ...
    private static func testKeywordPolicy() {
        ...
        check("synthetic UltraCMD events are classified as ignored", syntheticInput == .ignored)
    ...
    private static func testKeywordListenerLifecycle() async {
        ...
        listener.processEvent(
            ...
            secureEventInputEnabled: false)
        check(
            "real user input invalidates pending automatic delivery while UltraCMD events do not",
            activityCount == 1)
...
final class ClipboardManager {
    static let internalType = NSPasteboard.PasteboardType("com.ultracmd.internal")
    func prepareForUltraCMDPasteboardMutation() {}
    func synchronizeAfterUltraCMDPasteboardMutation(changeCount: Int) {}
}
...
enum Paster {
    static let ultracmdEventTag: Int64 = 0x54494E59
