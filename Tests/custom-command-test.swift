...
struct CustomCommandTests {
    ...
    static func main() async {
        ...
        setsid()
        let suiteName = "com.ultracmd.custom-command-tests"
        ...
        let scriptDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("ultracmd-scripts-\(UUID().uuidString)")
        ...
        let forwarded = await ShellCommandRunner.run(
            imported?.command ?? "", arguments: ["; touch /tmp/ultracmd-import-should-not-exist"],
            workingDirectory: imported?.workingDirectory)
        check(
            "an imported script receives its argument as one inert word",
            forwarded.standardOutput == "; touch /tmp/ultracmd-import-should-not-exist"
                && !FileManager.default.fileExists(atPath: "/tmp/ultracmd-import-should-not-exist"))
        ...
        let marker = await ShellCommandRunner.run("test \"$ULTRACMD\" = 1")
        check("the ULTRACMD marker is exported so a shell config can detect us", marker.succeeded)
        ...
        let injected = await collect(
            ShellCommandRunner.stream(
                "printf '%s\\n' \"$1\"", arguments: ["; touch /tmp/ultracmd-should-not-exist"]))
        check(
            "a value carrying shell syntax is data, not code",
            injected.log.contains("; touch /tmp/ultracmd-should-not-exist")
                && !FileManager.default.fileExists(atPath: "/tmp/ultracmd-should-not-exist"))
        ...
        let zdotdir = FileManager.default.temporaryDirectory
            .appendingPathComponent("ultracmd-zdotdir-\(UUID().uuidString)")
        try? FileManager.default.createDirectory(at: zdotdir, withIntermediateDirectories: true)
        try? Data("alias ultracmd_probe=true\n".utf8).write(
        ...
        let withEnvironment = await ShellCommandRunner.run(
            "ultracmd_probe", loadingShellEnvironment: true)
        check(
        ...
        let withoutEnvironment = await ShellCommandRunner.run("ultracmd_probe")
        check(
