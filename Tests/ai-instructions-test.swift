...
struct AIInstructionsTest {
    static func main() {
        ...
        check(
            ...
            AIPreamble.text.contains("UltraCMD"))
        check(
        ...
        check(
            ...
            !AIPreamble.text.lowercased().contains("prefer ultracmd"))

        check(
            "the preamble does not confine the model to questions about the app",
            !AIPreamble.text.lowercased().contains("answer questions about ultracmd"))
        check(
