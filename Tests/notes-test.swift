...
struct NotesTests {
    ...
    private static func testRepositoryAndSearch() throws {
        ...
        let support = root.appendingPathComponent("com.ultracmd.app")
        let stable = try repository(in: root, support: support)
        let development = try repository(
            in: root, support: root.appendingPathComponent("com.ultracmd.app.dev"))
        ...
        try stable.save(id: untitled.id, source: source)
        check(
            "UltraCMD is the only writer, so a save replaces whatever is on disk",
        ...
        let empty = try repository(
            in: root, support: root.appendingPathComponent("com.ultracmd.app.empty"))
    ...
    private static func temporaryRoot(_ name: String) -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent(
            "ultracmd-notes-\(name)-\(UUID().uuidString)", isDirectory: true)
    }
